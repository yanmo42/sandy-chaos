...
|----------|-------|--------|--------|--------|
| 1 | Complex entropy state needs physical derivation | High | Transforms the entire formal core | ⚠️ [AUD-012] Marked resolved but reopened by audit-2026-06-10; see findings AUD-001/-002/-005. Do not rewrite history — annotate only. ⚠️ **Reopened by 2026-06-10 audit** (AUD-005: §11 conflates arc-length and contour integrals; Cauchy diagnostic invalid as defined) — was ✅ via `math_foundations_zf.md` §10 |
| 2 | Show GR does actual theoretical work (not just aesthetics) | High | Justifies the project's distinctive premise | ⚠️ [AUD-012] Marked resolved but reopened by audit-2026-06-10; see findings AUD-001/-002/-005. Do not rewrite history — annotate only. ⚠️ **Reopened by 2026-06-10 audit** (AUD-001/-006: T-015 measures affine parameter on null geodesics, not proper time; flat baseline is a sign-mismatched strawman; no downstream consumer) — was ✅ via `math_foundations_zf.md` §9 |
...
| 6 | Rename/rethink "tachyonic loop" claims | Low | Removes internal contradiction | ⚠️ [AUD-012] Marked resolved but reopened by audit-2026-06-10; see findings AUD-001/-002/-005. Do not rewrite history — annotate only. ⚠️ **Partially reopened by 2026-06-10 audit** (rename was correct; AUD-005 shows the replacement Cauchy/winding bridge is itself broken under the stated arc-length definition) — was ✅ via `math_foundations_zf.md` §11 |
...
## 10. Audit Findings (2026-06-10): Problems to Fix
...
- [ ] **AUD-006 (S2):** Kerr layer has zero downstream consumers (`VortexChannel` uses hand-set `backward_attenuation=0.5`). Wire a Kerr-derived asymmetry profile in, or execute the A-005 downscope.
- [ ] **AUD-007 (S2):** Narrative–boundary coupling `q(L,t)=B₀+λN_t` (docs/05 §3) is unimplemented; toy model's "temporal asymmetry" is a constant bias. Implement the coupling; test corridor-structure survival.
- [ ] **AUD-008 (S1):** Hard-gate enforcement is honor-system (`validate_foundations.py` only catches self-declared violations; policy promotion is a frequency gate). Require adversarial re-derivation + comparator audit for any PASS transition.
