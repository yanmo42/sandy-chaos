...
- orchestrator cycle summaries now render the Lux–Nyx-routed disposition/target pair rather than the pre-routing heuristic, so archive and promotion-queue outcomes stay legible before dispatch
- cycle-summary rendering now reuses the already-generated task contracts instead of re-running Lux–Nyx shaping, so pilot counters (`state/lux_nyx/metrics.json`) are not double-counted by summary generation side effects
- when Lux–Nyx shaping emits a governance routing outcome, orchestrator contracts now use that outcome to steer archive/promotion routing before dispatch: `archive`/`refusal-log` downgrade to `LOG_ONLY` + `log-only`, `hold-queue` parks work in `TODO_PROMOTE` + `todo`, and `promotion-queue` can lift otherwise local work into a review-gated docs/workflow/tests-config promotion path; if Lux–Nyx provides only an explicit routed disposition, the workflow infers the matching routed target before enforcing promotion gates; downstream dispatch then skips archive/hold work and refuses promotion-queue docs/workflow/tests-config runs until review status is explicitly `approved`
...
- `memory/orchestrator_cycle_summary.md`
- `memory/self_improve_state.json` (repeated policy tweaks may now carry optional governance-routing metadata; self-improve doc promotion respects the same Lux–Nyx archive/hold/promotion-queue gates before appending workflow or agent policy tweaks, and approved routed promotion targets now steer those tweaks onto the matching durable surface such as `WORKFLOW.md` or `FOUNDATIONS.md` instead of relying only on keyword guesses)
- `state/lux_nyx/metrics.json` + `state/lux_nyx/pilot_report.json` (pilot measurement surfaces for the Lux–Nyx interaction layer: raw suggestion/archive counters, the three headline metrics — suggestion acceptance rate, correction burden per suggestion, archive→promotion conversion quality — and frozen pre-Lux–Nyx baseline comparisons; `pilot_report.json` now also carries the comparison policy metadata (`min_pilot_sample_size`, metric directionality, and the canonical headline metric list) plus the derived `promotion_verdict`, so downstream readers can interpret provisional vs stable baseline deltas without reconstructing that policy out of code; `scripts/lux_nyx_pilot_baseline.py` updates the comparison baseline without resetting live counters, can attach causal baseline provenance (`baseline_source`, `baseline_observed_at`) for auditability, and reports stay explicitly `unconfigured` until that freeze step happens so no false lift is claimed against a synthetic zero baseline; full-pass notifications now include a compact Lux–Nyx baseline snapshot sourced from `state/lux_nyx/pilot_report.json`, including the current verdict, suggestion-resolution coverage, unresolved suggestion count, and the per-metric deltas)
