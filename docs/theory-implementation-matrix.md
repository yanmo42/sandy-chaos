...
| T-018 | Actualized intelligence should be evaluated as closed-loop verified causal leverage per constrained resource, not raw model capability or speculative substrate imagery | C,E,S | Declared objective improves over baseline through a sense-model-decide-act-measure-update loop, with resource/risk vector, verification method, reversibility class, and failure condition declared before evaluation | C1, I2, P3, O3, E3, E4, A2 | `docs/21_closed_loop_causal_leverage.md`, `docs/00_sandy_chaos_blueprint.md`, `docs/glossary.md`, `spine/concepts/SC-CONCEPT-0010.yaml` | causal leverage card benchmark (to add); `python3 scripts/spine_validate.py` for spine linkage | causal leverage card report (to add) | New conceptual bridge; needs one mundane workflow card before it can govern speculative substrate-control scenarios | Low | REVIEW |
