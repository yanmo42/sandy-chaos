...
| T-015 | Kerr geometry produces intrinsic proper-time asymmetry unattainable in flat spacetime, justifying the GR layer as load-bearing rather than aesthetic | F,E | Proper-time asymmetry `(τ_prograde - τ_retrograde) / mean(τ)` > Sagnac-flat rotating baseline across spin parameters a/M ∈ [0.1, 0.9] with failable spin-curve criteria and step-size error bars | P1, P2, E3, E4, A2 | `docs/math_foundations_zf.md` §9, `cosmic_comm/` simulation code, validation script | `python3 scripts/validate_foundations.py --payload-file memory/research/kerr_asymmetry_2026-03/t015_rebuild_evidence.json`; rebuild: `python3 research/kerr_asymmetry_validation_v2.py` | `memory/research/kerr_v2_results_20260614.json`, `memory/research/kerr_asymmetry_2026-03/t015_rebuild_evidence.json` | 2026-06-10 audit AUD-001/-002 resolved by Contract 2 rebuild. Rebuilt using 4D timelike orbits (H = -0.5) and integrated proper times compared to Langevin-Minkowski Sagnac-flat baseline with matched coordinate-time asymmetry. Kerr proper-time asymmetry curve remains clearly distinguishable and exceeds the step-size numerical error bars. | High | PASS |
