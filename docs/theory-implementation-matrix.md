...
| T-012 | Hyperstition narrative dynamics $\mathcal{G}$ are testable via a two-agent mean-field toy model with fixed-point classification | C,E | Fixed points satisfy $m^*=f(m^*)$ and stability by $|f'(m^*)|<1$; paradox regimes must be reproducible under bounded temporal asymmetry | C1, I1, E2, A2 | `nfem_suite/intelligence/cognition/hyperstition.py`, `docs/05_hyperstition_temporal_bridge_analysis.md` | `python -m unittest tests.test_hyperstition_dynamics -q && python scripts/hyperstition_ablation_compare.py --out-dir memory/research/hyperstition-v0 --stem ablation_comparison_v0` | `tests/test_hyperstition_dynamics.py`, `plans/hyperstition_level4_result_packet_v0.md` | Executable tests passed in this environment and the current toy family now has a compact Level-4 result packet backed by null and ablation comparisons; remaining risk is robustness/generalization, not basic testability | High | PASS |
...
| T-018 | Actualized intelligence should be evaluated as closed-loop verified causal leverage per constrained resource, not raw model capability or speculative substrate imagery | C,E,S | Declared objective improves over baseline through a sense-model-decide-act-measure-update loop, with resource/risk vector, verification method, reversibility class, and failure condition declared before evaluation | C1, I2, P3, O3, E3, E4, A2 | `docs/21_closed_loop_causal_leverage.md`, `docs/22_causal_leverage_card_protocol.md`, `nfem_suite/intelligence/leverage/`, `scripts/run_leverage_card.py`, `tests/test_leverage_card.py`, `spine/concepts/SC-CONCEPT-0010.yaml` | `python3 -m unittest tests.test_leverage_card -q`; `python3 scripts/run_leverage_card.py memory/research/leverage/ --emit-evidence --quiet`; `python3 scripts/validate_foundations.py --payload-file memory/research/leverage/2026-06-06-003-leverage-harness-introduction.evidence.json` | `memory/research/leverage/2026-06-06-001-kerr-asymmetry-attestation.json` + `.evidence.json` (T-015, retrospective); `memory/research/leverage/2026-06-06-002-hyperstition-robustness-attestation.json` + `.evidence.json` (T-012, retrospective); `memory/research/leverage/2026-06-06-003-leverage-harness-introduction.json` + `.evidence.json` (T-018, prospective, self-referential) | Harness now exists with 17 unit tests and the first three cards score PASS (two retrospective attestations + one self-referential prospective). Self-referential PASS stands only until an independent prospective card on an unrelated mundane workflow is added. | Med | PASS (provisional) |
...
5. Author one independent prospective leverage card on a mundane workflow unrelated to the harness itself, so T-018 can move from `PASS (provisional)` to `PASS`.
