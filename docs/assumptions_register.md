...
| A-004 | **Proper-time asymmetry is an operationally relevant observable for directional communication metrics.** | Physical, Empirical | **DOWNSCOPED 2026-06-14** | `docs/02_tempo_tracer_protocol.md`, `docs/05_hyperstition_temporal_bridge_analysis.md`, channel metrics docs/code | Supports directional capacity surfaces and lead/lag-budget language. | The project can still study asynchronous observers, but relativistic distinctiveness weakens. T-015 REVIEW: affine-parameter proxy and sign-mismatched comparator mean the proper-time evidence base is non-discriminating pending Contract-2 rebuild. | Downscope | P1, I2, E4; T-002, T-015 |
| A-005 | **Kerr / curved-spacetime structure contributes something theoretically load-bearing beyond generic flat-space timing asymmetry.** | Physical, Modeling | **DOWNSCOPED 2026-06-14** | `docs/02_tempo_tracer_protocol.md`, `docs/math_foundations_zf.md` | Justifies why curved geometry is more than aesthetic packaging. | Kerr treated as illustrative/special-case only. Contract-2 must rebuild T-015 with an invariant observable, Sagnac baseline, and failable spin-curve criterion before this assumption can be reinstated as load-bearing. | Downscope | P1, O3; T-005, T-015 |
| A-006 | **The complex entropy state** $Z = \alpha + i\beta$ **is a useful canonical embedding for the order/disorder state space.** | Formal / Math, Modeling | Adopted, Implemented | `docs/math_foundations_zf.md`, `docs/math_appendix.md`, `nfem_suite/formalization/set_theoretic/`, `nfem_suite/formalization/complex_euler.py` | Enables magnitude/phase language, winding diagnostics, and a compact state representation. The set-theoretic chain through ℚ is now constructive in code, with the ℝ-crossing labelled. | Replace with a real vector/two-channel representation; keep the phenomena if they survive without complex notation. | Redirect | O2, O3, C4; T-016 |
...
| A-012 | **Empirical success requires lift over declared baselines with reproducible metrics.** | Empirical, Governance | Adopted, Open | `docs/prediction-protocol.md`, `docs/09_research_automation_protocol.md`, `docs/theory-implementation-matrix.md` | Gives the project a scientific success criterion rather than a vibes-based one. | ⚠️ Consequence class should be reconsidered per audit-2026-06-10; A-002 fallback (ordinary forecasting/latency effects) currently describes actual evidential state — cross-link audit-2026-06-10-physical-theory-validity.md. | Downscope | E1–E5, I2, A1 |
...
| A-015 | **The ℝ-bridge crossings (B-001 IEEE-754, B-002 trapezoidal, B-003 atan2 branch cut) are explicitly enumerated as named approximation assumptions; every float-returning helper cites which assumptions it depends on.** | Computational, Governance | Implemented | `nfem_suite/formalization/set_theoretic/bridge.py`, `tests/test_set_theoretic_chain.py`, `docs/math_foundations_zf.md` §15 | Makes the ℚ→ℝ approximation boundary inspectable. Tests pin the assumption registry, so a regression that drops or renames a crossing fails CI. | Bridge crossings revert to being implicit in numpy/library calls; auditing where exactness ends becomes harder, but the underlying behaviour does not change. | Downscope | O2, O3, A2, C4; T-016 |
| A-016 | **Actualized intelligence only counts when a loop produces measured causal leverage under declared resource, risk, reversibility, and verification denominators.** | Modeling, Computational, Governance | Adopted, Open | `docs/21_closed_loop_causal_leverage.md`, `docs/00_sandy_chaos_blueprint.md`, `docs/glossary.md` | Prevents "applied intelligence" from becoming prestige language and creates a bridge between observer coupling, energistics, phase sift, and workflow evaluation. | Downscope actualized-intelligence language to glossary only until a causal leverage card can separate real intervention gain from drift, luck, or unlogged human action. | Redirect | C1, I2, P3, O3, A2; T-018 |
| H-1 | **'Downstream in space' ↔ 'future in time' mapping exists for some observer class.** | Physical, Modeling | Open, Speculative | `docs/02_tempo_tracer_protocol.md`, `docs/05_hyperstition_temporal_bridge_analysis.md` | Grounds all "future-like" claims on a concrete observer symmetry; makes geodesic/light-cone structure operationally legible. | Future-like claims become observer-agnostic language only; temporal asymmetries may not generalize across observer classes. | Downscope | C1, O3; T-002, T-005 |
| H-2 | **Affine-parameter differences along null geodesics proxy proper-time or channel timing.** | Physical, Modeling | Open, Partial | `docs/02_tempo_tracer_protocol.md` | Provides a coordinate-free way to measure lead/lag along causal paths. | The proxy may not hold under finite-interaction backaction; proper-time and affine-parameter decouple in some regimes. | Redirect | P1, O3, A-004; T-015 |
| H-3 | **Comparator classes in discriminating tests contain the strongest mundane alternative.** | Empirical, Governance | Open | `docs/09_research_automation_protocol.md`, `docs/prediction-protocol.md` | Ensures a null hypothesis is never weaker than the actual alternative present in data. | Predicted advantage may be an artifact of comparison selection rather than genuine framework advantage. | Quarantine | E1–E5, A-012; T-015 |
| H-4 | **Parabolic diffusion adequately stands in for the finite-c_up hyperbolic mechanism, including finite delay.** | Modeling, Computational | Open, Partial | `docs/math_foundations_zf.md`, `nfem_suite/simulation/temporal/` | Simplifies numerical integration and tractability while preserving qualitative structure. | Signal propagation delay and nonlinear steepening may not be captured; hyperbolic regimes are underrepresented. | Redirect | P1, O3, A-005; T-015 |
| H-5 | **MI (mutual information) about a boundary implies decision-relevant anticipatory advantage.** | Modeling, Empirical | Open, Speculative | `docs/21_closed_loop_causal_leverage.md`, `docs/09_research_automation_protocol.md` | Justifies why Shannon entropy is a proxy for forecasting lift. | Boundary information may be latency-based only; no causal advantage in closed-loop decision or control. | Downscope | I2, A-012, A-002; T-015 |
| H-6 | **Automation-gate honesty: hard-gate enforcement assumes proposers declare their own violations.** | Governance | Open, Partial | `docs/00_sandy_chaos_blueprint.md`, `FOUNDATIONS.md` | Avoids false negatives by trusting human auditors to surface their own boundary/metric stretches. | Unknown violations hide; gates become performative rather than meaningful. | Quarantine | A2, A-011, A-013; T-007 |
...
- **A-015 fails** → the ℝ-bridge becomes opaque again; the underlying numerical behaviour is unchanged, but the audit-where-exactness-ends affordance is lost. This is a docs/code visibility regression, not a metaphysical one.
- **A-016 fails** → "actualized intelligence" remains a useful phrase at most, not a measurement or governance primitive.
