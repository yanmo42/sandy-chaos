...
The baseline is now an explicit frozen surface rather than an implicit all-zero default: use `python3 scripts/lux_nyx_pilot_baseline.py --suggestion-acceptance-rate <x> --correction-burden-per-suggestion <y> --archive-to-promotion-conversion-quality <z>` to write the pre-Lux-Nyx comparison reference without resetting live counters. For promotion decisions, freezing unlabeled numbers is not enough — the baseline should also carry causal provenance (`--baseline-source`, `--baseline-observed-at`) so the pilot is compared against a traceable pre-Lux-Nyx observation rather than a detached placeholder.
...
Until that freeze step happens, `state/lux_nyx/pilot_report.json` marks baseline comparison entries as `direction: "unconfigured"` with `baseline` / `delta` left null, so the pilot does not claim false lift against a synthetic zero baseline. Each comparison row also reports `sample_size`, `sample_sufficient`, and `sample_remaining` so the operator can see both whether a direction read is still provisional and exactly how many more causal observations are needed before that metric clears the pilot sample gate. The same report now also exposes `counts.unresolved_suggestions` plus `measurement_health.suggestion_resolution_coverage`, making it explicit when acceptance/correction rates are still being diluted by surfaced suggestions that have not yet been causally resolved.
